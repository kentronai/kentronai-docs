...
In the sidebar, the **MCP Gateway** area lists only **Workspaces** while you are at gateway level, and the area heading points at the same place — the landing page has no sidebar entry of its own. Once you open a workspace the area lists only **All workspaces**. Clicking any row in either table opens that workspace's **Overview**.
...
- A wrench, labelled `<name> has tools enabled`, when the workspace has at least one connected integration. Despite the label, the test is connection presence, not the per-connection allowlist — that lives with the connection itself, and reading it would cost a request per row. [Tools and permissions](/mcp-gateway/tools-and-permissions) is where you see what is actually published.
...
<Frame caption="Organization settings → Workspaces, with the Create workspace dialog open over the list. Note the submit button here is Save workspace, and that the Default row is marked as active and carries the wrench icon for a workspace that has a connected integration.">
  <img src="/images/mcp-gateway/new-workspace.png" alt="The Workspaces page behind a Create workspace dialog containing a Workspace name field, a Cancel button and a Save workspace button." />
</Frame>
