...
<Frame caption="The chat home page as an owner or admin sees it: a greeting, the composer with its + menu, Skills chip and a rotating example prompt, a Replay button top right, and the Docs link and Beetle runners count in the sidebar footer. The recent-chat list under Chat is a scroll area, so its last row is clipped at the bottom edge. Members see a shorter sidebar.">
