...
<Frame caption="The Usage page on a Free plan. Only the Members tile carries a denominator, so its ring is the only one with any fill — a short green arc for 1 of 5 seats — while the Workspaces and Integrations rings stay empty whatever they count. Recorded spend need not match Credit used: spend sums the cost recorded on every request, including turns that used your own provider key, while Credit used is only what the platform credit balance has given up.">
  <img src="/images/llm-gateway/usage.png" alt="The Usage page on a Free plan: a Workspace picker set to Default, a Current plan card reading Free with an Active pill, then a Free plan · Default grid with an Upgrade button holding three tiles — Workspaces 6 and Integrations 0 beside empty grey rings, and Members 1 / 5 beside a ring with a short green arc. Below them, headline cards read Requests this period 39, Total tokens 540,325 and Credit used $0.12, a Credits and spend row reads Credit remaining $4.88, Recorded spend $0.43 and Top model openai/gpt-5.6-luna, and a Token breakdown row reads Input 169,155, Output 20,120, Reasoning 3,977 and Cache-read 5,851." />
</Frame>
...
<Note>
The credit message points at a page called "Billing & Credits". No page ships under that name — the destination is **Billing**, described on [Billing and plans](/core/billing-and-plans). The envelope these refusals are returned in is on [Gateway configuration](/llm-gateway/configuration), and every user-visible chat message is quoted on [Errors and limits](/co-worker/errors-and-limits).
</Note>

## The request rate limit

Chat is limited to **30 requests per 60-second window, per user**. It is a fixed-window counter in Postgres, keyed on the user and the window start; there is no per-organization limit, no per-key limit, and no `Retry-After` header — the wait hint travels inside the error envelope as `retryAfterSeconds`. The window and the maximum are deployment variables, listed on [Gateway configuration](/llm-gateway/configuration).

<Warning>
**Setting `VITE_DISABLE_REDIS=true` turns this limiter off completely, and the cloud environment template ships that value** — so a deployment copied from it starts with no request limit. [Gateway configuration](/llm-gateway/configuration) explains why.
</Warning>
