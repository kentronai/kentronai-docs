...
<Warning>
`BYOK_ENCRYPTION_KEY_B64` is an AES-256-GCM key. Do not rotate it once organizations have saved keys: no re-encryption routine exists anywhere in the codebase and decryption hard-fails on any other key version, so every stored key becomes permanently undecryptable and each organization must re-enter its key. The same applies to `RECEIPT_CONNECTION_ENCRYPTION_KEY_B64` and stored connection bundles. See [Bring your own key](/llm-gateway/bring-your-own-key).
</Warning>
...
| A newly published table shows no rows | A live replica is never backfilled for a table added to the publication after it was built | `zero:reset`, which drops and recreates the publication, deletes `zero.db*` and so forces a full initial sync; also clear browser site data |
| zero-cache exits shortly after start, following a schema reset | zero-cache reuses `apps/start/zero.db` across resets and replays `CREATE TABLE` against tables that already exist in the replica | Delete `zero.db*`, or let `zero:reset` do it |
| `relation "<schema>.receipt_job_projection" does not exist` during `zero:migrate` | The Zero migration, which refreshes `zero_data`, ran before Receipt's durable projection tables existed in the configured schema | The supervisor pre-migrates the Receipt schema first; if you drive the migration by hand, use the same ordering |
| `Migration <file> was already applied with a different checksum. Create a new migration file instead of editing old ones.` | An applied migration file was edited in place | Restore it and add a new timestamped file instead |
