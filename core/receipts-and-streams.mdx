...
Underneath, a receipt is an immutable record appended to a named stream. A stream is a hash-linked chain of receipts. Everything else in Receipt — job state, objective state, chat history — is a **fold** over one of those chains: the current value is not stored anywhere authoritative, it is computed by replaying the chain from the start and applying each receipt in turn.
