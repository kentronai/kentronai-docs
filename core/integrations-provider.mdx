...
<Warning>
**`RECEIPT_CONNECTION_ENCRYPTION_KEY_B64` cannot be rotated.** Stored connection bundles are stamped with a key version and the reader hard-fails on any other version; there is no re-encryption routine. Change the key and every stored connection reference becomes permanently undecryptable, with re-entry as the only recovery. The same is true of `BYOK_ENCRYPTION_KEY_B64`. Both are covered in [Configuration, secrets, and keys](/core/configuration).
</Warning>
