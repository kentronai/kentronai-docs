...
Publishing has a scope of its own, `factory/objectives/<objectiveId>/publish`.

<Note>
**Memory search is keyword matching in this release.** The memory layer can rank entries by embedding similarity, but no call site supplies it with an embedding function — not the runtime, not the in-repo CLI, not the Factory service — so every search a worker or a [runtime memory route](/core/runtime-api#memory) performs matches on the terms in the stored text. `summarize` is a character-capped join of the matched entries, not a model-written summary.
</Note>
