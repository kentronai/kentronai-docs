...
`GET /healthz` returns `200` whatever that check said. It carries `ok`, `ready`, `degraded`, `uptimeSec`, `dataDir`, `processRole`, a `queue` snapshot (`version`, `total` and counts for `queued`, `leased`, `running`, `completed`, `failed` and `canceled`), `postgres`, `codexBin` and `resonateUrl`. The comment in the handler states the intent: `Health is a liveness surface, not a replay boundary.` It is deliberately bounded so that slow queue repair or receipt replay cannot make a healthy runtime look down.

`GET /readyz` returns `200` or `503` from the same check, with a smaller body: `ok`, `ready`, `degraded`, `uptimeSec`, `processRole` and `postgres`. The `postgres` object carries `ok` and a `target` naming the host, database, user and `sslmode` it tried, plus an `error` holding the connection message when it failed — enough to tell a wrong host from a wrong password without printing either.
