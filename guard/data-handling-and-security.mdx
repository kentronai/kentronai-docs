...
Every action through the REST call route (`POST /connect/call`) writes a `tool.called` receipt and, when the call succeeds, a `tool.observed` receipt into a per-workspace stream. Output stored in a receipt is truncated to 2,000 characters with a `truncated` flag. A failed call gets only `tool.called`, carrying its error. Receipt-writing failures are logged and swallowed rather than being allowed to fail a working tool call.

<Warning>
**Calls made through the MCP bridge are not yet recorded.** The JSON-RPC endpoint behind the bridge (`POST /connect/mcp`) enforces the same scope and allowlist checks, but writes no `tool.called` or `tool.observed` receipt. Do not treat gateway receipts as a complete record of tool activity until that gap closes. [Receipts and audit](/guard/receipts-and-audit) lists everything that does and does not write a receipt.
</Warning>
...
Zero data retention is an **organization compliance policy**, not an infrastructure setting. The toggle, `Require ZDR (Zero Data Retention)`, describes itself as `Only allow models from AI providers that do not retain data. This is enforced at the provider level.`, and its help line reads `Applies immediately to model availability.` When it is on:
...
- **Tool access is gated.** The tool policy adds a `blocked_by_compliance` reason. In this release that reason applies to one tool — Anthropic code execution — and only while the organization has no active executable Anthropic key.
...
<Warning>
**The companion flag, `Require organization provider key`, is not enforced on the server.** It has a toggle and a description — `Only allow models from providers that have an active organization API key.` — and a real client-side effect: the chat model picker hides every model whose route providers lack an active executable organization key. But the server-side compliance check reads only the ZDR flag, so a request that names a hidden model is not rejected. Treat it as a picker filter, not a boundary.
</Warning>
