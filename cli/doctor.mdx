...
Both probes are plain `GET` requests with a ten-second timeout, and neither follows redirects: a host that answers `/health` with a `301` or `302` reads as unhealthy rather than as reachable.
