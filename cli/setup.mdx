...
`receipt setup` is how you sign the released CLI in. It is also reachable as `receipt connect setup`. The similarly named `receipt connect doctor` and `receipt connect check` no longer run setup — they run the read-only [`receipt doctor`](/cli/doctor), which resolves your endpoint and reports the saved session without ever opening a browser.
...
For the hosted app there is nothing to configure — the binary resolves `https://app.kentron.ai` on its own, so a plain `receipt setup` on a fresh machine signs you straight in. You only pass a host when you are pointing at a dev or self-hosted origin; see [Install the CLI](/cli/install).
...
To sign out, run [`receipt logout`](/cli/overview) — it deletes the active `session.json` and its matching per-target file, and prints `receipt logout: not signed in` when there was nothing to remove. Deleting the files by hand still works too.
...
The connector list is the whole catalog, not the connectors you have connected. To see actual connections, use `receipt connect status`.
