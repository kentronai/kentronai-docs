...
Everything your connections publish is callable from the terminal. `receipt tools` lists the tools your workspace can use, shows what one takes, and runs it. `receipt mcp` points another client — Codex, or anything that speaks MCP over stdio — at the same set, without writing a credential into that client's configuration.
...
`receipt tools call --connection <connection>` goes to `POST /connect/call`, which writes `tool.called` and `tool.observed` receipts and appears on the workspace's [Gateway activity](/mcp-gateway/gateway-activity) dashboard.

<Warning>
**Aggregate calls are not recorded as tool receipts in this release.** `receipt tools call` without `--connection`, and every call an MCP client makes through `receipt mcp serve`, go to `POST /connect/mcp`, which does not write `tool.called` or `tool.observed`. Those calls do not show up on the Gateway activity dashboard. If you need a recorded trail for a specific call, make it with `--connection`.
</Warning>
