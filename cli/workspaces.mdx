...
<Tip>
If `receipt tools list` shows fewer tools than you expected, check `receipt workspace current` first. `receipt tools list`, `receipt tools call` and `receipt mcp config` all read the workspace bound into your saved session. `receipt connect status` does not: it always reports the Default workspace's connections, which is why the two can disagree.
</Tip>
