...
<Note>
A saved token is only ever sent to its own gateway host — the origin it was minted for. It is never forwarded to a different `--server-url` or environment gateway.
</Note>
