...
| `Stopped the response, but could not cancel the agent run.` | The stream closed, but the cancel request to the runtime failed. This one appears in the banner, not in the thread, and a runtime that returned its own error message shows that message instead. | Cancel the run from the [Tasks page](/co-worker/objectives-and-tasks), where the confirmation reads **Cancel task**. |
