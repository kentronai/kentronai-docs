...
2. **Rate limit.** 30 requests per minute per user, applied only when Redis is enabled.
...
The router is a single model call whose only job is to classify. Its system prompt is one sentence: "You are the Beetle chat router. Return one routing decision only; do not perform tool work and do not write any user-facing answer, greeting, acknowledgement, or status update." (Beetle is the assistant's name in the interface.) It returns one JSON decision and nothing else, so no words it produces reach your thread.
...
Enabled skills are mounted as a read-only tree with a compact catalog beside it: files at mode `0444`, directories at `0555`, replaced only by the controller between packets, so a run already under way does not see a later enable or disable. Receipt does not rank skills for the agent; the catalog skill tells it to "Select the smallest relevant set using the task, plan, and catalog metadata" and to read each chosen skill completely first. A patch touching that tree is rejected as a Receipt operational path, and no receipt records which skill was read. See [Skills](/catalog/skills).
