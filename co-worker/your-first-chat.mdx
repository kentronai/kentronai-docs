...
When you send, Receipt first decides what kind of turn this is. A question it can answer from the conversation gets a streamed reply in the thread. A request that needs current data from your connected systems, a file, code, or anything outside the chat becomes a [background run](/co-worker/background-runs). A request to write a new organization skill gets a draft you can review. While that decision is being made, the thread shows a status line above the reply — `Sending your message to Beetle...` first, then whatever the run reports. Beetle is the assistant's name in the interface. The three routes are described in [What the AI Co-Worker does](/co-worker/overview), and the pipeline behind them in [How it works](/co-worker/how-it-works).
...
Beside `+` sits a **Skills** chip. It is how you tell a run to follow one of your organization's skills — the versioned instruction files an owner or admin has added on the Skills page.

<Frame caption="The Skills chip open on a new chat. Under the heading Use a skill, each enabled skill is listed by name with its slug underneath; six are shown here, the menu's maximum. Create skill and Manage skills are further down the same menu, below the edge of this capture.">
  <img src="/images/co-worker/composer-skills-menu.png" alt="The Skills chip menu in the Receipt composer, listing six enabled organization skills with their slugs under the heading Use a skill." />
</Frame>

- Under **Use a skill**, pick a skill and its slug is appended to whatever you have already typed as `/slug`, separated by a space from the text in front of it and followed by another — it composes with a half-written prompt rather than replacing it. Type the rest of your request around it and send as usual.
- Only skills that are active and enabled appear, and any member of the organization can pick one. The menu shows at most six; when there are more, a final line counts the rest — `3 more on the Skills page` — and opens the page.
- With nothing enabled, the menu says `No enabled skills yet.`
- **Create skill** opens the Skills page and asks the browser for its upload picker straight away, so you land on the file chooser rather than on a list; if the browser refuses to open a file dialog it did not initiate, the page's **Add skill** button is focused instead. **Manage skills** opens the page itself. Both destinations are owner and admin only; a member who picks them never sees the page or an access-denied screen — the settings route sends them to `/`, which puts a signed-in user back in Chat.

The slug is ordinary text in your message. Receipt does not treat it as a command: the router and any background run read it like the rest of your prompt. What a run does with it, and how to have Receipt draft a new skill from the conversation, is on [Skills in chat](/co-worker/skills-in-chat); the files themselves are described under [Skills](/catalog/skills).
