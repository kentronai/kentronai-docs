...
A completed result carries no link back into Receipt. To see what the run did, open the [Tasks page](/co-worker/objectives-and-tasks), where it sits with the rest of your [background runs](/co-worker/background-runs), and use [Replay](/co-worker/replay). When the run produced no readable answer, the text is `Receipt objective <status>.` or `Receipt run <status>.`
...
Every message is handled on its own. Receipt does record the conversation as a thread — in a channel, the reply chain under the root post; elsewhere, the whole conversation — and appends each later message to it. But earlier messages never reach the router or the run, and no run is bound to the conversation, so a follow-up about an earlier run starts a fresh one rather than reacting to it. Write follow-ups as complete requests, and use the web app to react to or cancel a run; see [Background runs](/co-worker/background-runs).
