...
Consistent with that scope list, none of the following are implemented: slash commands, interactive buttons or modals, direct messages, an App Home tab, file uploads, or message shortcuts. There is no approval prompt in the thread either ([tools and permissions](/mcp-gateway/tools-and-permissions)). The entire surface is: get mentioned in a channel, and reply in that channel's thread.
...
The link opens the run in the web app, where [Replay](/co-worker/replay) shows every step. It is also the way back into a Slack run: the Tasks page lists only objectives started on that page, so one started from Slack is not there. See [background runs](/co-worker/background-runs).
...
That stream is what makes the follow-up behaviour and the restart recovery above possible: context is replayed from receipts rather than kept in memory, and only a successful delivery clears a pending one. The run itself is an ordinary background run on the chat lane, which is why Replay rebuilds it exactly like a run started from web chat. See [Receipts and streams](/core/receipts-and-streams) for the model behind it.
