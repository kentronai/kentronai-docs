...
<Frame caption="The Inventory tab after one AWS scan. Both discovered EC2 workloads are classified L4 and Critical because no owner could be resolved, and the line above the table records the last scan.">
  <img src="/images/catalog/agent-registry-inventory.png" alt="The Agent Registry inventory table listing two AWS EC2 agent workloads with autonomy level L4, critical risk and an Unowned owner column." />
</Frame>

The columns are a row number, **Agent** (provider badge, name, and `AWS · us-east-1` underneath), **Platform**, **Function**, **Level**, **Risk**, **Owner**, **Credential**, **Last used** and **Actions**. Clicking an agent — or **View details** in its row menu — opens the register entry. **Search agents** matches name, resource id, platform, function, owner and region; the **Autonomy** button filters by level; the table pages client-side at 10, 25, 50 or 100 rows. Above it, `Showing 2 of 2 agents` sits opposite `Last scan: AWS · completed · 7 Sept 2026, 19:32`, which reads **"No scans yet"** before the first scan and carries a warning triangle when the last one failed. Every timestamp on this page is rendered in UTC, with the date written the way your browser's locale writes it.
...
**Connect** opens the provider's credential form inside the dialog, under the line "Credentials are collected and stored by Nango. Receipt keeps only the connection reference."
</Step>
...
<Frame caption="The Dashboard tab, computed from the whole inventory rather than the Inventory tab's current search and filter. Every bar shows a count and a share of the total.">
...
Five counts — **Total agents discovered**, **Critical risk**, **High risk**, **Agents with no owner**, **Dormant (30+ days)** — and three distributions: **Agent distribution by autonomy level**, **Ownership status**, and **Credential types**, each row reading `2 · 100%`. The credential label is a constant per provider rather than a per-agent observation: `IAM role` for AWS, `OAuth token` for Google Cloud, `Service principal` for Azure, and `No instance profile` for an EC2 instance without one. Before the first scan the tab reads **"Connect a cloud account and scan it to build the executive risk dashboard."** There are no trends and no per-provider breakdown.
