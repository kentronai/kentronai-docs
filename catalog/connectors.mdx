...
The organization-wide surface is **Integrations** in the organization settings rail, titled **Global Integrations**:
...
The six category chips replace the catalog's own fine-grained categories — 23 of them in Receipt's curated list — which the code calls "far too many chips to scan".
...
<Note>
Cloud accounts connected for the [Agent Registry](/catalog/agent-registry) use a dedicated connection named `agent-registry` and are deliberately filtered out of this page, so a scan credential never reads as — or gets reused as — your organization's primary connection to that cloud. Connecting AWS, Azure or Google Cloud here is a separate act: it publishes that connector's own tools to chat and MCP clients, which a registry scan credential never does.
</Note>
...
**Tool surface** decides what a connection publishes. The five surfaces are explained in full on [Receipt Connect](/mcp-gateway/receipt-connect); in short:

| Surface | What the connection publishes |
| --- | --- |
| `typed-proxy` | Reviewed, checked-in tools with fixed methods, closed schemas and an explicit read or write class |
| `compatibility-read` | One Receipt-owned tool, `read-provider-resource` — provider-relative `GET` only |
| `resource-aware-get` | One read tool, for Atlassian connectors, where Receipt resolves the site and rewrites the path |
| `provider-mcp` | Tools discovered from the provider's own MCP server, deliberately excluded from the aggregate gateway |
...
<Note>
`gcp` sits in two places at once. Its manifest declares both a command-backed credential mode and proxy tools, and the classifier checks proxy tools first — so `gcp` reports as `typed-proxy` while still producing an isolated `gcloud` configuration inside a worker.
</Note>
