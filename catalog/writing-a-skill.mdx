...
Every enabled skill is mounted for a run and listed in a compact catalog carrying each skill's name, description, version, content hash and path. Mounting follows the run's authorization: runs started from chat, Slack and Teams carry your organization, while an objective started without an organization context — from the in-repo developer CLI, for example — receives no organization skills at all. The run's own context tells the agent so, in these words:
...
Nothing ranks those entries or matches them to the request. One of Receipt's own skills, copied into every run, states the intended reading discipline — read the catalog, "select the smallest relevant set using the task, plan, and catalog metadata", do not read every `SKILL.md` merely because it is mounted, and read each selected file completely before acting on it. Nothing in the run's prompt points the agent at that file, so whether it follows the discipline on any given run is its decision, not a rule Receipt enforces.
...
**Keep one skill to one job.** A short file that gets opened is worth more than a manual that gets skipped, and the reading discipline above explicitly discourages loading everything.
...
**Do not ask a skill to change its own surroundings.** The mounted catalog is controller-owned and read-only; the skill that states that discipline also tells the agent never to edit it, copy it into a patch, or report it as its own output, and — whatever the agent does — a run rejects a patch that touches those paths.
