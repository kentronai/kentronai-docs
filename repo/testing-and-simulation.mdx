...
| `./bunw run check:full` | `check:fast`, then the builds — `build:web` and `build:receipt`. |
...
| `./bunw run sst:config:test` | 17 explicitly named Bun test files under `deploy/sst/`, `scripts/` and `tests/smoke/validate-stack.test.ts` — the deployment and configuration contract suite. |
...
| `./bunw run receipt:test:smoke` | `bun run build`, then `bun test ./tests/smoke --max-concurrency=1 --timeout=$RECEIPT_SMOKE_TIMEOUT_MS`, then the mock-LLM proxy integration test in a fresh process. There are 64 files in `tests/smoke`. |
...
| `withSimulationDateNow(...)` | Patches the ambient `Date.now` for one scope |
| `createDeterministicSimulationRandom(...)` | A seeded PRNG |
| `createDeterministicSimulationEntropy({...})` | A labelled, domain-separated, replayable entropy tape with `float`, `int` and `choice` draws |
| `createDeterministicSimulationIdSource(...)` | Deterministic queue, job, command and event ids |
| `withDeterministicSimulationEnvironment(...)` | Patches `Date.now`, `Math.random` and optionally timers for one scope |
| `traceSimulationEvent(...)` | Appends a trace event |
| `createDeterministicSimulationScheduler({...})` | Turns `setTimeout` and `setInterval` into ordered simulation steps |
...
| `DEFAULT_DETERMINISTIC_SIMULATION_MAX_STEPS` | The default step limit, `10 000` |

Ordering of steps scheduled for the same instant is configurable: `"fifo"`, `"lifo"`, `"actor"`, `"label"`, or `{ kind: "seeded", seed }`.
...
**Named sub-suites:** `search`, `corpus add|reduce|promote`, `useful-gate`, `generic-agent-loop`, `projection-ui`, `prod-replay`, `deterministic-runtime`, `runtime-outbox`, `runtime-status-polling`, `projection-serving-fairness`, `funding-settlement`, `cross-channel-ingress`, `self-improvement`, `reliability-suite`.
...
| `--coverage-floor <metric>=<value>` | built-in floors | Repeatable. The metric must be one of the built-in coverage keys; a negative or non-numeric value gives `--coverage-floor <metric>=<value> must use a non-negative number`. |
...
<Steps>
  <Step title="add">
    `receipt factory simulate corpus add --seed <n> [--corpus-file <target>] [--reason <text>]` records the seed. A missing seed gives `factory simulate corpus add requires --seed <non-negative-number>`. The text output prints `Corpus file`, `Seed`, `Added` and `Corpus seeds`.
  </Step>
  <Step title="reduce">
  ...
  <Step title="promote">
    `receipt factory simulate corpus promote --input <reduced-corpus.json> [--corpus-file <target>]` makes it permanent. It refuses an unverified corpus: `factory simulate corpus promote requires a verified reduced corpus; pass --allow-unverified to override`.
  </Step>
</Steps>
...
Scenarios are JSON files under the eval scenario root, with `software/` and `computer-use/` subdirectories. A scenario resolves from an absolute path, a repo-relative path, `<root>/<id>.json`, or an id matched by walking both roots. Every subcommand syncs the eval-run and computer-use-session projections before it does anything else.

`report` prints a scorecard: `Receipt Eval Report`, `Runs`, `Pass rate`, `Abstraction miss rate`, `Revert rate`, `Strong handoff completeness rate`, then one line per run. `replay` runs a `receipt dst` audit scoped to that run's stream.
...
Error strings are exact: `eval subcommand is required`, `eval run requires a scenario id or path`, `eval inspect requires a run id`, `eval replay requires a run id`, `eval run '<id>' not found`, `Unknown eval subcommand '<x>'`.
...
`validate:stack` runs `scripts/validate-stack.sh`, which exercises a running stack end to end rather than a package in isolation. `--help` prints its full usage block.
...
Next step: [return to local development](/repo/local-development).
