...
- **Return actions.** `actions` returns an array built from `action`, `assistant`, `tool` or `human`. Each carries a `run` function; `when` and `sideEffects` are both optional. See [the TypeScript SDK and the action contract](/develop/typescript-sdk) for the full contract.
...
Prerequisites: a `defineAgent` spec at `<repo root>/src/agents/<id>.agent.ts`, Postgres, and Resonate.
...
|---|---|
| `action.selected` → `action.started` → `action.output.manifest` → `action.completed` | A normal action pass. The manifest carries the action's buffered receipt output; for an action declaring `sideEffects: "query"` that output is fenced and de-duplicated by `invocationId`. |
| `action.superseded` | The action's `commitWhen` fence returned false inside a CAS retry, evaluated against both the selected view and the latest view. The loop reselects from the new head instead of committing. |
| `human.requested` with no matching `human.responded` | A `human` action is waiting. The controller blocks until a correlated domain receipt makes the action's `responseWhen` predicate true. |
...
| `goal.completed`, then `run.completed` | The goal predicate is satisfied. |
| No further `action.selected` | Selection found nothing runnable; the scheduler's reason for that selection is `settled`. |

Selection itself is deterministic. Runnable actions are sorted by kind priority — `human`, then `assistant`, then `action`, then `tool`. If any selected action is `exclusive`, exactly that one is selected and the reason is `exclusive`; otherwise the batch is capped by the lowest `maxConcurrency` among the selected actions and the spec default, with the reason `concurrency-cap` or `priority-order`.
...
The control policy version is `runtime-policy-v2` and the scheduler policy version is `scheduler-v2`.
...
Next step: [check your work with the test and simulation suites](/repo/testing-and-simulation).
